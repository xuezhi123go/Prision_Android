			...
			switch (msg.what) {
				case WHAT_REQCHAIRMAN:
					...
					if (!VConferenceManager.isChairMan()) {
						Toast.makeText(GKApplication.getInstance(), "申请主席失败", Toast.LENGTH_SHORT).show();
					}
					break;
				case WHAT_REQSPEAKER:
					// 申请主讲失败
					if (!VConferenceManager.isSpeaker()) {
						Toast.makeText(GKApplication.getInstance(), "申请主讲失败", Toast.LENGTH_SHORT).show();
					}
					break;
...
//				VConferenceManager.inviteVConfPersonWin();
			...
			case R.id.exit_Img:
				if(mCancelVideoDialog==null){
					mCancelVideoDialog=new CancelVideoDialog(getActivity(),true);
	...
	public void setQuietImageView(final boolean quiet) {
		...
		getView().post(new Runnable() {
			...
			public void run() {
				if (quiet) {
					mQuietView.setCompoundDrawablesWithIntrinsicBounds(null, GKApplication.getInstance().getResources().getDrawable(R.drawable.vconf_mute_selector), null,
							null);
				} else {
					mQuietView.setCompoundDrawablesWithIntrinsicBounds(null,GKApplication.getInstance().getResources().getDrawable(R.drawable.vconf_speaker_selector),
							null, null);
				}
	...
	public void setMuteImageView(final boolean mute) {
		...
		getView().post(new Runnable() {
			...
			public void run() {
				if (mute) {
					mDumbView.setCompoundDrawablesWithIntrinsicBounds(null, GKApplication.getInstance().getResources()
							.getDrawable(R.drawable.vconf_microphone_off_selector), null, null);
				} else {
					mDumbView.setCompoundDrawablesWithIntrinsicBounds(null,
							GKApplication.getInstance().getResources().getDrawable(R.drawable.vconf_microphone_on_selector), null, null);
				}

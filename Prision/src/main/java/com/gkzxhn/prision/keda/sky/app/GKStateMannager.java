...
public class GKStateMannager {
	...
	public void registerGK(String achE164, String password, String domain, long gkIp, String terlAlias) {
		...
		if (NetWorkUtils.isAvailable(GKApplication.getInstance())) {
			Log.i("Login flow", "GK 注册" + "  achDomain-" + domain + "  achNumber-" + achE164 + "  dwIp-" + gkIp);
			mRegisteringGK = true;
			mRegisterGK = false;
			Base.registerGK(mtCSUAddrApi);

			setRegisterGKBind(GKApplication.getInstance(), mtCSUAddrApi);
			mE164 = achE164;
		} else {
	...
	public void registerGK() {
		...
		TMtCSUAddrApi mtCSUAddrApi = getRegisterGKFromSharedPreferences(GKApplication.getInstance());

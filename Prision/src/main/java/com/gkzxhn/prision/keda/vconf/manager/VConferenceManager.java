...
public class VConferenceManager {
	...
	public static int confCallRete(Context context) {
		if (context == null) {
			context = GKApplication.getInstance();
		}
...
//				Toast.makeText(MyApplication.getContext(), "GK重连中...", Toast.LENGTH_SHORT).show();
		...
		if (isCalling() || EmNativeConfType.isCalling(nativeConfType)) {
			if (toastInfo) {
				Toast.makeText(GKApplication.getInstance(), "正在呼叫中。。。", Toast.LENGTH_SHORT).show();
			}
			return false;
		}
		...
		if ((isCallIncoming())) {
			if (toastInfo) {
				Toast.makeText(GKApplication.getInstance(), "正在被叫中。。。", Toast.LENGTH_SHORT).show();
			}
			return false;
		}
		...
		if (EmNativeConfType.isConveneing(nativeConfType)) {
			if (toastInfo) {
				Toast.makeText(GKApplication.getInstance(), "正在召集会议。。。", Toast.LENGTH_SHORT).show();
			}
			return false;
		}

...
//			Toast.makeText(GKApplication.getInstance(), "APS登录失败:"+ finalApsErr, Toast.LENGTH_SHORT).show();
		...
		userLogin.achConfigPath = GKApplication.getInstance().getFilesDir().getAbsolutePath();
...
//									"错误码" + resultType);
	...
	public static void parseImLoginRsp(JSONObject jsonBodyObj) {
		...
		try {
			...
			if (preImHandle == 0) {
				SharedPreferences mPreferences = GKApplication.getInstance().getSharedPreferences("com.kedacom.truetouch_preferences", Context.MODE_PRIVATE);
